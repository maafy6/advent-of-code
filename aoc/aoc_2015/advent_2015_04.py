...
def find_prefix(secret: str, prefix: str) -> int:
    """Determine the first number that generates the prefix.
    ...
    while True:
        i += 1
        if md5(f"{secret}{i}".encode()).hexdigest()[: len(prefix)] == prefix:
            return i
...
def part1(data: str = DATA) -> int:
    ...
    :returns: The first number that satisfies the prompt.
    """
    return find_prefix(data, "00000")
...
def part2(data: str = DATA) -> int:
    ...
    :returns: The first number that satisfies the prompt.
    """
    return find_prefix(data, "000000")
