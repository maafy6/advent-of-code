...
Note: Backslashes are escaped in the docstring, so a double backslash in the
challenge text below, `\\`, should be read as a single backslash.
...
def get_lengths(code: str) -> Tuple[int, int]:
    """Get the code and string lengths.

    :param code: The code string.
    :returns: A tuple containing the code and string lengths.
    """
    code = code.strip()
    ...
    while i < len(code) - 2:
        ...
        if code[i] == "\\":
            if code[i + 1] in ("\\", '"'):
                ...
                i += 3
        str_len += 1
    return len(code), str_len


def get_encoding_lengths(code: str) -> Tuple[int, int]:
    """Get the code and encoded lengths.

    :param code: The code string.
    :returns: A tuple containing the code and encoded lengths.
    """
    code = code.strip()
