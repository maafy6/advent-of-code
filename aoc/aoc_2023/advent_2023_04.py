...
def score_scratcher(
    winning: Sequence[str], card: Sequence[str], *, count: bool = False
) -> int:
    """Score a scratcher card.

    :param card: The numbers on the card.
    :param winning: The winning numbers.
    :param count: Return the number of matching numbers instead of the score.
    :returns: The score for the card or the number of matching numbers.
    """
    matches = len(set(winning).intersection(card))
    return matches if count else (1 << matches) // 2


def gen_cards(data: str) -> Iterator[Tuple[Sequence[str], Sequence[str]]]:
    ...
    for card in data.splitlines():
        ...
        yield winning.split(), card_numbers.split()
...
def part1(data: str = DATA) -> int:
    ...
    """
    return sum(score_scratcher(w, n) for w, n in gen_cards(data))
...
def part2(data: str = DATA) -> int:
    ...
    for i, (w, n) in enumerate(gen_cards(data)):
        num_copies = count[i]
        for j in range(score_scratcher(w, n, count=True)):
            count[i + j + 1] += num_copies
