...
So, the next value of the first history is `18`. Finding all-zero differences
for the second history requires an additional sequence:
...
So, the next value of the second history is `28`. The third history requires
even more sequences, but its next value can be found the same way:
...
So, the next value of the third history is `68`. If you find the next value for
each history in this example and add them together, you get `114`. Analyze your
OASIS report and extrapolate the next value for each history. What is the sum of
...
def get_deltas(data: Sequence[int]) -> list[int]:
    """Get the deltas of a sequence of numbers.

    :returns: A list of length one less the input sequences, the entries of
        which are the deltas between successive entries in the input list.
    """
...
def predict(data: Sequence[int], *, reverse: bool = False) -> int:
    """Predict the next element in the sequence.

    :param data: The current data.
    :param reverse: Instead of predicting the next entry, predict what would
      have come before the first entry.
    ...
    deriv = get_deltas(data)
    if all(d == 0 for d in deriv):
        return data[-1]

    return data[-1] + predict(deriv)
...
def part1(data: str = DATA) -> int:
    ...
    return sum(predict(r) for r in reports)
...
def part2(data: str = DATA) -> int:
    ...
    """
    reports = parse_input(data)
    return sum(predict(r, reverse=True) for r in reports)
