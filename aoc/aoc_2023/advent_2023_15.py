...
- If there is not already a lens in the box with the same label, add the lens to
    ...
    the other lenses when you do this. If there aren't any lenses in the box,
...
def holiday_hash(s: str) -> int:
    """Compute the Holiday ASCII String Helper value.

    :param s: The input string.
    :returns: The hash value.
    """
    return functools.reduce(lambda a, b: ((a + ord(b)) * 17) % 256, s, 0)


def initialization_sequence(seq_str: Iterable[str]) -> dict[int, dict[str, int]]:
    """Create the initialization sequence map.

    :param seq_str: The list of instructions.
    :returns: The initialization map.
    """
    # This implementation relies on the fact that dict keys in Python are
    # ordered by their insertion order.
    initialization = defaultdict(dict)
    for instruction in seq_str:
        if instruction[-1] == "-":
            label = instruction[:-1]
            try:
                del initialization[holiday_hash(label)][label]
            except KeyError:
                pass
        else:
            label, lens = instruction.split("=")
            initialization[holiday_hash(label)][label] = int(lens)

    return initialization


def validate_init(initialization: dict[int, dict[str, int]]) -> int:
    """Validate the initialization sequence map.

    :param initialization: The initialization sequence map.
    :returns: The validation value for the sequence map.
    """
    return sum(
        (hash_val + 1) * (slot + 1) * focal_length
        for hash_val, lenses in initialization.items()
        for slot, (_, focal_length) in enumerate(lenses.items())
    )
...
def part1(data: str = DATA) -> int:
    ...
    """
    return sum(holiday_hash(s) for s in data.split(","))
...
def part2(data: str = DATA) -> int:
    ...
    :returns: The solution.
    """
    initialization = initialization_sequence(data.split(","))
    return validate_init(initialization)
