...
`2`. The other cards stay in the same order: `A`, `K`, `Q`, `T`, `9`, `8`, `7`,
`6`, `5`, `4`, `3`, `2`, `J`.
...
class Hand:
    ...
    def __lt__(self, other: "Hand") -> bool:
        ...
        if self.type == other.type:
            for i in range(5):
                if self.__card_rank.index(self.hand[i]) != self.__card_rank.index(
                    other.hand[i]
                ):
                    return self.__card_rank.index(
                        self.hand[i]
                    ) < self.__card_rank.index(other.hand[i])
    ...
    @property
    def type(self) -> int:
        """Classify the hand according to the rules."""
        counts = [len(list(g)) for _, g in groupby(sorted(self.hand))]
        if len(counts) == 1:
            return 7
        if len(counts) == 2:
        ...
        if len(counts) == 5:
            return 1
        raise ValueError("Unrecognized hand.")


class JokerHand(Hand):
    """Represent a hand with the joker rule."""

    __card_rank = "J23456789TQKA"

    @property
    def type(self) -> int:
        """Classify the hand based on how to treat the jokers."""
        return self.jokered().type

    def jokered(self) -> Hand:
        """Return a hand where jokers have been replaced by their most advantageous type."""
        if "J" not in self.hand:
            return Hand(self.hand)

        counts = sorted(
            [(k, len(list(g))) for k, g in groupby(sorted(self.hand)) if k != "J"],
            key=lambda c: c[1],
            reverse=True,
        )
        if len(counts) == 0:
        ...
        return Hand(self.hand.replace("J", counts[0][0]))


def parse_input(data: str, cls: type[Hand]) -> list[tuple[Hand, int]]:
    """Parse the input.

    :param data: The input data.
    :returns: A list of tuples of (hand, bid).
    """
    hands = []
    for line in data.splitlines():
        hand, bid = line.split()
        hands.append((cls(hand), int(bid)))
...
def part1(data: str = DATA) -> int:
    ...
    """
    hands = sorted(parse_input(data, Hand), key=lambda v: v[0])
...
def part2(data: str = DATA) -> int:
    ...
    :returns: The solution.
    """
    hands = sorted(parse_input(data, JokerHand), key=lambda v: v[0])
    return sum((i + 1) * h[1] for i, h in enumerate(hands))
